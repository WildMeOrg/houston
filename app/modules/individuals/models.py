...
class Individual(db.Model, HoustonModel, CustomFieldMixin):
    ...
    def update_autogen_names(self, user):
    ...
    def set_autogenerated_name_species(self, agn, user):
        if not agn or agn.type != 'auto_species':
            return
        tx_guid = self.get_taxonomy_guid()
        if not tx_guid or str(tx_guid) != str(agn.reference_guid):
            return
        # FIXME TODO handle existing taxonomy names
        new_name = agn.get_next()
        self.add_name(f'autogen-{agn.guid}', new_name, user)
