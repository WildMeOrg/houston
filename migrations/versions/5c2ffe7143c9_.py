...
def upgrade():
    ...
    with op.batch_alter_table('asset_group_sighting', schema=None) as batch_op:
        batch_op.add_column(sa.Column('asset_group_guid', app.extensions.GUID(), nullable=False))
        batch_op.add_column(sa.Column('meta', sa.JSON(), nullable=True))
        batch_op.drop_index('ix_asset_group_sighting_owner_guid')
        batch_op.create_index(batch_op.f('ix_asset_group_sighting_asset_group_guid'), ['asset_group_guid'], unique=False)
        batch_op.drop_constraint('fk_asset_group_sighting_owner_guid_asset_group', type_='foreignkey')
        batch_op.create_foreign_key(batch_op.f('fk_asset_group_sighting_asset_group_guid_asset_group'), 'asset_group', ['asset_group_guid'], ['guid'])
...
def downgrade():
    ...
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('asset_group_sighting', schema=None) as batch_op:
        batch_op.add_column(sa.Column('owner_guid', sa.CHAR(length=32), nullable=False))
        batch_op.drop_constraint(batch_op.f('fk_asset_group_sighting_asset_group_guid_asset_group'), type_='foreignkey')
        batch_op.create_foreign_key('fk_asset_group_sighting_owner_guid_asset_group', 'asset_group', ['owner_guid'], ['guid'])
        batch_op.drop_index(batch_op.f('ix_asset_group_sighting_asset_group_guid'))
        batch_op.create_index('ix_asset_group_sighting_owner_guid', ['owner_guid'], unique=False)
