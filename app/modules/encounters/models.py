...
class Encounter(db.Model, FeatherModel):
    ...
    owner_guid = db.Column(db.GUID, db.ForeignKey('user.guid'), index=True, nullable=False)
    owner = db.relationship('User', backref=db.backref('owned_encounters'), foreign_keys=[owner_guid])

    submitter_guid = db.Column(db.GUID, db.ForeignKey('user.guid'), index=True, nullable=True)
    submitter = db.relationship('User', backref=db.backref('submitted_encounters'), foreign_keys=[submitter_guid])
