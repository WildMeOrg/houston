...
AUTOGEN_NAME_CONTEXT_PREFIX = 'autogen-'
...
class AutogeneratedName(db.Model, HoustonModel):
    ...
    # this will ensure that a prefix will not get used more than once within a type
    __table_args__ = (db.UniqueConstraint(prefix, type),)
    ...
    @property
    def context(self):
        return f'{AUTOGEN_NAME_CONTEXT_PREFIX}{self.guid}'

    @property
    def reference(self):
        if self.type == AutogeneratedNameType.auto_species:
            from app.modules.site_settings.models import Taxonomy

            # will raise ValueError if invalid
            return Taxonomy(self.reference_guid)
        else:
            raise ValueError(f'unsupported reference type {self.type}')
    ...
    def resolve_value(cls, name):
        agn_guid = name.autogenerated_guid
        if not agn_guid:
    ...
    def validate_names(cls, new_autogen_names, skip_taxonomy_check=False):
        ...
        for new_agn_guid in new_autogen_names:
            ...
            if not re.search('^[a-zA-Z0-9]+$', new_agn['prefix']):
                raise HoustonException(
                    log,
                    f'Prefix {new_agn["prefix"]} invalid, must use only characters a-z, A-Z, and 0-9',
                )
            ...
            pt = f"{new_agn['prefix']}:{new_agn['type']}"
            if pt in prefix_type:
                raise HoustonException(
                    log,
                    f"Prefix {new_agn['prefix']} cannot be used more than once for type {new_agn['type']}",
                )
            prefix_type.append(pt)
            # test taxonomy value
        ...
        if enabled_taxonomies:
            ...
            if len(enabled_taxonomies) != len(all_taxonomies):
                raise HoustonException(
                    log,
                    f'You must set/enable/disable Autogen names for all taxonomies; {all_taxonomies} vs {enabled_taxonomies}',
                )
            try:
                txs = Taxonomy.get_configuration_value()
            except ValueError:
                txs = []
    ...
    @classmethod
    def integrity_find_invalid_name_contexts(cls):
        from app.modules.names.models import Name

        names = Name.query.filter(
            Name.context.startswith(AUTOGEN_NAME_CONTEXT_PREFIX)
        ).all()
        return [n for n in names if not n.get_autogenerated_name()]

    @classmethod
    def integrity_find_invalid_references(cls):
    ...
    @classmethod
    def integrity_find_invalid_individuals(cls):
        from app.modules.individuals.models import Individual
        ...
        for indiv in indivs:
            ...
            for name in indiv.names:
                agn = name.get_autogenerated_name()
                if not agn:
                    continue
                if agn.type not in ct:
                    ct[agn.type] = 0
                ct[agn.type] += 1
                # for now, 2+ of same type is enough cuz only species-type in use  FIXME
                if ct[agn.type] > 1:
                    bad.append(indiv)
                    log.info(f'{name} for {agn} exceeds type count on {indiv}')
                    continue
        return bad
