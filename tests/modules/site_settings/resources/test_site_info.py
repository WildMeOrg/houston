...
import app.version
from tests.modules.site_settings.resources import utils as site_setting_utils
...
def test_site_info(flask_app_client):
    with mock.patch('flask.current_app.sage.get_dict') as sage_get_dict:
        ...
        resp = site_setting_utils.read_site_info(flask_app_client, None)
...
@pytest.mark.skipif(extension_unavailable('sage'), reason='Sage extension disabled')
def test_site_info_staff(flask_app_client, staff_user):
    with mock.patch('flask.current_app.sage.get_dict') as sage_get_dict:
        sage_get_dict.return_value = {
            'status': {
                'success': True,
        ...
        resp = site_setting_utils.read_site_info(flask_app_client, staff_user)
    assert resp.status_code == 200
    assert resp.content_type == 'application/json'
    assert resp.json == {
        'houston': {
