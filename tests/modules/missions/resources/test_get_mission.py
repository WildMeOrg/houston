...
@pytest.mark.skipif(module_unavailable('missions'), reason='Missions module disabled')
def test_get_mission_by_search(flask_app_client, admin_user):
    from app.modules.missions.models import Mission

    previous_list = mission_utils.read_all_missions(flask_app_client, admin_user)

    new_missions = []
    for index in range(3):
        nonce, title = mission_utils.make_name('mission')
        response = mission_utils.create_mission(flask_app_client, admin_user, title)
        mission_guid = response.json['guid']
        mission = Mission.query.get(mission_guid)
        new_missions.append((nonce, mission))

    current_list = mission_utils.read_all_missions(flask_app_client, admin_user)
    ...
    for nonce, new_mission in new_missions:
        mission_utils.delete_mission(flask_app_client, admin_user, new_mission.guid)
