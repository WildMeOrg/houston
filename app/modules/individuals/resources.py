...
# TODO FIXME this should deprecate cuz it should be automagic now
@api.route('/autogen_species_name/<uuid:individual_guid>')
@api.response(
    code=HTTPStatus.NOT_FOUND,
    description='Individual not found.',
)
@api.resolve_object_by_model(Individual, 'individual')
class IndividualAutogenSpeciesName(Resource):
    @api.permission_required(
        permissions.ObjectAccessPermission,
        kwargs_on_request=lambda kwargs: {
            'obj': kwargs['individual'],
    ...
    def post(self, individual):
        if individual is not None:
            ...
            log.error('autogen_species_name called for nonexistent Individual')
            return {}
