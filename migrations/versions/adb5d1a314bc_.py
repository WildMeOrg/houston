...
def upgrade():
    ...
    # note: sqlite requires we start out with a default value, then drop it.   :(
    with op.batch_alter_table('annotation', schema=None) as batch_op:
        batch_op.add_column(sa.Column('ia_class', sa.String(length=255), nullable=False, server_default='unknown'))
        batch_op.add_column(sa.Column('bounds', sa.JSON(), nullable=False, server_default='{}'))
