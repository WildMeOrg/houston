...
class Individual(db.Model, HoustonModel, CustomFieldMixin):
    ...
    def add_autogenerated_species_name(self, user):
        from app.modules.autogenerated_names.models import AutogeneratedName

        tx_guid = self.get_taxonomy_guid()
        if not tx_guid:
            return
        autogen_names = AutogeneratedName.query.filter_by(reference_guid=tx_guid).all()
        if autogen_names and len(autogen_names) == 1:
            new_name = autogen_names[0].get_next()
            self.add_name('autogen-species', new_name, user)
