...
class AutogeneratedName(db.Model, HoustonModel):
    ...
    def validate_names(cls, new_autogen_names):
        ...
        for new_agn_guid in new_autogen_names:
            ...
            if new_agn['type'] == AutogeneratedNameType.auto_species.value:
                try:
                    Taxonomy(new_agn['reference_guid'])
                except ValueError:
                    raise HoustonException(
                        log,
                        f"Invalid taxonomy guid {new_agn['reference_guid']}",
                    )
                if new_agn.get('enabled', False):
    ...
    def set_names_as_rest(cls, new_autogen_names):
        ...
        for new_agn_guid in new_autogen_names:
            ...
            if existing_agn:
                re_enabled = not existing_agn.enabled and new_agn.get('enabled', False)
                with db.session.begin(subtransactions=True):
                    existing_agn.next_value = new_agn.get('start_value', 0)
                    existing_agn.prefix = new_agn['prefix']
                    existing_agn.enabled = new_agn.get('enabled', existing_agn.enabled)
                ...
                with db.session.begin(subtransactions=True):
                    ...
                    autogenerated_name = AutogeneratedName(
                        ...
                        enabled=new_agn.get('enabled', True),
