...
class Individual(db.Model, HoustonModel, CustomFieldMixin):
    ...
    def merge_names(self, source_individuals, override=None, fail_on_conflict=False):
        ...
        override_contexts_agn = set()
        if isinstance(override, dict):
            for ctx in override.keys():
                if ctx.startswith(AUTOGEN_NAME_CONTEXT_PREFIX):
                    override_contexts_agn.add(ctx)
                else:
                    override_contexts.add(ctx)
        contexts_on_self = set()
        names_on_self_agn = set()
        for name in self.names:
            if name.autogenerated_guid:
                name._agn = name.get_autogenerated_name()
                names_on_self_agn.add(name)
            else:
                contexts_on_self.add(name.context)
        log.debug(
            f'merge_names({self}) => override_contexts={override_contexts} override_contexts_agn={override_contexts}'
        )
        log.debug(
            f'merge_names({self}) => contexts_on_self={contexts_on_self} names_on_self_agn={names_on_self_agn}'
        )

        # this handles non-AGN stuff
        for indiv in source_individuals:
        ...
        for ctx in override_contexts_agn:
        ...
        for name in names_on_self_agn:
            if str(name._agn.type.value) in use_for_type:
                needs_historical.append(name)
            else:
                use_for_type[str(name._agn.type.value)] = name
        # non-override for source_individuals: only for historical (cannot be used for type, unless via override)
        for indiv in source_individuals:
            for name in indiv.names:
                if not name._agn:
                    continue
                type = str(name._agn.type.value)
                if type in use_for_type and str(use_for_type[type].guid) != str(
                    name.guid
        ...
        if tx_name and (str(tx_name._agn.reference_guid) != str(self.taxonomy_guid)):
            raise ValueError(
        ...
        log.debug(
            f'merge_names({self}) => use_for_type={use_for_type} needs_historical={needs_historical}'
        )
