...
def test_merge_names(
    ...
    # check that we can override with 2nd indiv name + taxonomy
    #    this would have been done by override.taxonomy_guid
    indivs[0].taxonomy_guid = indivs[1].taxonomy_guid
    agn_name_guid = indivs[1].names[0].guid  # this should be the only one we end up with
    override = {
        str(indivs[1].names[0].context): str(agn_name_guid),
    }
    indivs[0].merge_names([indivs[1]], override)
    for iname in indivs[0].names:
        if iname.autogenerated_guid:
            assert iname.guid == agn_name_guid
